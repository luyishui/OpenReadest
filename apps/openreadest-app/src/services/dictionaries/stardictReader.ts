...
const normalizeCharset = (value?: string): string => {
  ...
  // 顺序重要：utf-16le/be 必须在 utf-16 之前精确命中。
  const supported: Array<[tag: string, label: string]> = [
    ['utf-8', 'utf-8'],
    ['gb18030', 'gb18030'],
    ['gbk', 'gbk'],
    ['gb2312', 'gbk'],
  ...
  const hit = supported.find(([tag]) => raw.includes(tag));
  return hit?.[1] ?? 'utf-8';
};
...
export class StarDictReader {
  ...
  /**
   * `.ifo` 声明的 charset（默认 utf-8）。`.idx`/`.syn` 词条与 `.dict` 释义
   * 都按此编码解码——GBK 等非 UTF-8 中文词典（第三方工具常见）也能正确查询。
   */
  charset = 'utf-8';
  private entryDecoder: TextDecoder = decoder;
  ...
  async load(opts: StarDictReaderOpts): Promise<void> {
    ...
    this.charset = normalizeCharset(this.ifo['charset']);
    this.entryDecoder = new TextDecoder(this.charset);
  ...
  private async decodeIdxEntry(i: number): Promise<StarDictEntry> {
    ...
    const word = this.entryDecoder.decode(bytes.subarray(0, nullPos));
  ...
  /**
   * 前缀建议：返回最多 `limit` 个以 `prefix` 开头（大小写不敏感）的词条。
   * 用于查无结果时的"相近词提示"。
   *
   * `.idx` 按字节序（strcmp 序）排序，而大小写变体（USA/us/use）在此序下
   * 并不相邻——大小写不敏感二分不可靠。这里用与排序一致的字节序二分定位
   * 首个 >= prefix 的位置，再向前/向后双向扫描收集（大小写变体通常落在
   * 边界附近），保证混合大小写词表（iPhone、USA、专有名词）不丢候选。
   */
  async suggest(prefix: string, limit = 5): Promise<string[]> {
    const trimmed = prefix.trim();
    if (!trimmed || this.idxCount === 0) return [];
    const lower = trimmed.toLowerCase();

    // 字节序 lower-bound（与 .idx 排序一致，可靠）
    let lo = 0;
    let hi = this.idxCount - 1;
    let start = this.idxCount;
    while (lo <= hi) {
      const mid = (lo + hi) >>> 1;
      const entry = await this.decodeIdxEntry(mid);
      if (entry.word < trimmed) lo = mid + 1;
      else {
        start = mid;
        hi = mid - 1;
      }
    ...
    // 向前扫描（大小写变体常落在字节序边界之前），上限 50 条
    const back = 50;
    for (let i = Math.max(0, start - back); i < start && out.length < limit; i++) {
      push((await this.decodeIdxEntry(i)).word);
    }
    // 向后扫描：大小写不敏感匹配无法在字节序上精确 break，保守扫描到
    // limit 满或达到扫描上限（有 idxCache，实际代价低）
    const MAX_SCAN = 200;
    for (let i = start; i < this.idxCount && out.length < limit && i - start < MAX_SCAN; i++) {
      push((await this.decodeIdxEntry(i)).word);
    }
  ...
  private async decodeSynEntry(i: number): Promise<{ syn: string; idxIndex: number }> {
    ...
    let nullPos = 0;
    while (nullPos < bytes.length && bytes[nullPos] !== 0) nullPos++;
    const syn = this.entryDecoder.decode(bytes.subarray(0, nullPos));
