 ...
 * files, gzip files without an RA subfield, or malformed headers that would
 * otherwise throw a RangeError (those need whole-file gunzip).
 */
export function parseDictZipHeader(bytes: Uint8Array): DictZipMeta | null {
  ...
  if (12 + xlen > bytes.length) return null; // extra region overruns the buffer
  let chlen = 0;
  let chcnt = 0;
  ...
  while (p + 4 <= 12 + xlen) {
    ...
    if (p + 4 + slen > 12 + xlen) return null; // subfield overruns the extra region
    if (si1 === 0x52 && si2 === 0x41) {
      ...
      if (p + 10 + 2 * chcnt > 12 + xlen) return null; // chunk size table overruns
...
async function probeChunkInflate(blob: Blob, meta: DictZipMeta): Promise<boolean> {
  ...
  // 真分块文件里每个非末块解压后必须恰好是 chlen（末块才允许更短）。
  // 连续 deflate 流（块间无 Z_FULL_FLUSH，现实中存在）的块 0 切片只解出
  // 部分输出（< chlen），会被此校验拒绝并回退到整文件 gunzip——
  // 否则后续块切片解出的是流中间垃圾字节，词条静默损坏。
  const inflateChunk = async (i: number): Promise<Uint8Array | null> => {
    let start = meta.compressedDataOffset;
    for (let k = 0; k < i; k++) start += meta.chunkSizes[k]!;
    const cs = meta.chunkSizes[i]!;
    const compressed = new Uint8Array(await blob.slice(start, start + cs).arrayBuffer());
    return inflateChunkStreaming(compressed);
  };

  const first = await inflateChunk(0);
  if (!first || first.length !== meta.chlen) return false;

  // 双块验证：连续流即使块 0 碰巧解满 chlen，块 1（流中间切片）也几乎不可能
  // 恰好解出 chlen。跳过末块——真分块的末块解压可以短于 chlen。
  const lastIndex = meta.chunkSizes.length - 1;
  if (lastIndex > 1) {
    const second = await inflateChunk(1);
    if (!second || second.length !== meta.chlen) return false;
  }
  return true;
}
...
export async function loadDictBody(blob: Blob, opts: LoadDictBodyOpts = {}): Promise<DictBody> {
  ...
  if (isGzip(head)) {
    try {
      ...
      // 畸形 FEXTRA/RA（或损坏的块数据）不应让整个查询失败——
      // 回退到整文件 gunzip；gunzip 失败则抛原始错误由上层处理。
    }
